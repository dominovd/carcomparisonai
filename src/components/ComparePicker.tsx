  ...
  return (
    <div className="mt-8 max-w-4xl rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-xl shadow-slate-200/70 backdrop-blur sm:p-4">
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] md:items-end">
        <div>
          <p className="mb-1.5 px-1 text-xs font-bold uppercase tracking-[0.18em] text-brand-dark">
            Car 1
          </p>
          <VehicleCombobox label="First car" value={a} onChange={setA} />
        </div>
        <span className="mb-2 hidden h-9 w-9 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-center text-xs font-bold uppercase tracking-wide text-ink-faint md:flex">
          vs
        </span>
        <div>
          <p className="mb-1.5 px-1 text-xs font-bold uppercase tracking-[0.18em] text-brand-dark">
        ...
        <button
          onClick={() => {
            window.location.href = `/compare/${buildPairSlug(a, b)}`;
          }}
          disabled={same}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 px-1">
        <p className="text-xs text-ink-faint">
          No signup required · Free comparison · {dataStatus.vehicles} vehicles available now
        </p>
        {same && (
